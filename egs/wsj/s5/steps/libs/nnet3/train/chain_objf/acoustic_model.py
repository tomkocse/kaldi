...
def combine_models(dir, num_iters, models_to_combine, num_chunk_per_minibatch,
    ...
    # We reverse the order of the raw model strings so that
    # the freshest one goes first.  This is important for systems that
    # include batch normalization-- it means that the freshest
    # batch-norm stats are used.  Since the batch-norm stats are
    # not technically parameters, they are not combined in the
    # combination code, they are just obtained from the first model.
